...
struct FixedBase6DoF9DoF : public testing::Test
{
    FixedBase6DoF9DoF()
    {
...
// 
TEST_F(FixedBase6DoF9DoF, TestForwardDynamicsContactsDirectSimple)
{
    Model model;
    model.gravity = Vector3d(0., -9.81, 0.);
...
TEST_F(FixedBase6DoF9DoF, TestForwardDynamicsContactsDirectMoving)
{
    Model model;
    model.gravity = Vector3d(0., -9.81, 0.);
    Body base_body(1., Vector3d(0., 0., 0.), Vector3d(1., 1., 1.));
...
)
{
    ...
    constraint_set.
            AddConstraint(contact_body_id, contact_point, contact_normal
    );
    ConstraintSet constraint_set_lagrangian = constraint_set.Copy();

    constraint_set_lagrangian.
            Bind(*model);
    constraint_set.
            Bind(*model);
...
//	cout << LogOutput.str() << endl;
    ...
    EXPECT_NEAR(constraint_set_lagrangian
                        .force[0], constraint_set.force[0], TEST_PREC);
    EXPECT_NEAR(contact_normal
                        .
                                dot(point_accel_lagrangian), contact_normal
                        .
                                dot(point_accel_contacts), TEST_PREC
    );
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(point_accel_lagrangian.data(), point_accel_contacts.data(), 3, TEST_PREC));
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(QDDot_lagrangian.data(), QDDot_contacts.data(), QDDot_lagrangian.size(), TEST_PREC));
}
...
TEST_F (FixedBase6DoF, ForwardDynamicsContactsSingleContactRotated)
{
    ...
    EXPECT_NEAR(contact_normal
                        .
                                dot(point_accel_lagrangian), contact_normal
                        .
                                dot(point_accel_contacts_opt), TEST_PREC
    );
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(point_accel_lagrangian.data(), point_accel_contacts_opt.data(), 3, TEST_PREC));
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(QDDot_lagrangian.data(), QDDot_contacts_opt.data(), QDDot_lagrangian.size(), TEST_PREC));
}
...
)
{
    ...
    constraint_set.
            AddConstraint(contact_body_id, Vector3d(1., 0., 0.), contact_normal
    );
    constraint_set.
            AddConstraint(contact_body_id, Vector3d(0., 1., 0.), contact_normal
    );
...
// check whether FDContactsLagrangian and FDContacts match
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(constraint_set_lagrangian.force.data(), constraint_set.force.data(), constraint_set.size(), TEST_PREC));
...
)
{
    ...
    constraint_set.
            AddConstraint(contact_body_id, contact_point, Vector3d(1., 0., 0.)
    );
    constraint_set.
            AddConstraint(contact_body_id, contact_point, Vector3d(0., 1., 0.)
    );
...
)
{
    ...
    constraint_set.
            AddConstraint(child_2_id, contact_point, Vector3d(0., 1., 0.)
    );
...
//	cout << "Lagrangian contact force " << contact_data_lagrangian[0].force << ", " << contact_data_lagrangian[1].force << ", " << contact_data_lagrangian[2].force << endl;
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(QDDot_lagrangian.data(), QDDot.data(), QDDot.size(), TEST_PREC));
}
...
)
{
    ...
    constraint_upper_trunk.
            AddConstraint(body_id_3dof[BodyUpperTrunk], Vector3d(1.1, 2.2, 3.3), Vector3d(1., 0., 0.)
    );
    constraint_upper_trunk.
            Bind(*model_3dof);
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(qddot_lagrangian.data(), qddot.data(), qddot_lagrangian.size(), TEST_PREC * qddot_lagrangian.norm() * 10.));
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(qddot_lagrangian.data(), qddot_sparse.data(), qddot_lagrangian.size(), TEST_PREC * qddot_lagrangian.norm() * 10.));
}
...
)
{
    ...
    ConstraintSet constraint_upper_trunk;
    constraint_upper_trunk.
            AddConstraint(body_id_3dof[BodyFootLeft], heel_point, Vector3d(1., 0., 0.)
    );
    constraint_upper_trunk.
            AddConstraint(body_id_3dof[BodyFootLeft], heel_point, Vector3d(0., 1., 0.)
    );
    constraint_upper_trunk.
            AddConstraint(body_id_3dof[BodyFootLeft], heel_point, Vector3d(0., 0., 1.)
    );
    constraint_upper_trunk.
            AddConstraint(body_id_3dof[BodyFootRight], heel_point, Vector3d(1., 0., 0.)
    );
    constraint_upper_trunk.
            AddConstraint(body_id_3dof[BodyFootRight], heel_point, Vector3d(0., 1., 0.)
    );
    constraint_upper_trunk.
            AddConstraint(body_id_3dof[BodyFootRight], heel_point, Vector3d(0., 0., 1.)
    );
    constraint_upper_trunk.
            Bind(*model_3dof);
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(Vector3d(0., 0., 0.).data(), heel_left_velocity.data(), 3, TEST_PREC));
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(Vector3d(0., 0., 0.).data(), heel_right_velocity.data(), 3, TEST_PREC));
}
