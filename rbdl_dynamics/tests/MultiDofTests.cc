...
struct SphericalJoint : testing::Test
{
    SphericalJoint()
    {
...
TEST_F(SphericalJoint, TestQuaternionIntegration)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(q_zyx_t1.data(), q_t1.data(), 4, 1.0e-5));
}
...
TEST_F(SphericalJoint, TestGetQuaternion)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEq(reference_1.data(), quat_1.data(), 4));

    Quaternion reference_3(-6., -7., -8., -9.);
    Quaternion quat_3 = multdof3_model.GetQuaternion(4, sphQ);
    EXPECT_TRUE(unit_test_utils::checkArraysEq(reference_3.data(), quat_3.data(), 4));
}
...
TEST_F(SphericalJoint, TestSetQuaternion)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEq(reference_1.data(), test.data(), 4));
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEq(reference_2.data(), test.data(), 4));
}
...
TEST_F(SphericalJoint, TestOrientation)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(emu_orientation.data(), sph_orientation.data(), 9, TEST_PREC));
}
...
TEST_F(SphericalJoint, TestUpdateKinematics)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(emulated_model.v[emu_body_id].data(), multdof3_model.v[sph_body_id].data(), 6, TEST_PREC));
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(emulated_model.a[emu_body_id].data(), multdof3_model.a[sph_body_id].data(), 6, TEST_PREC));

    UpdateKinematics(multdof3_model, sphQ, sphQDot, sphQDDot);

    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(emulated_model.v[emu_child_id].data(), multdof3_model.v[sph_child_id].data(), 6, TEST_PREC));
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(emulated_model.a[emu_child_id].data(), multdof3_model.a[sph_child_id].data(), 6, TEST_PREC));
}
...
TEST_F(SphericalJoint, TestDynamicsConsistencyRNEA_ABA)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(sphTau.data(), tau_id.data(), tau_id.size(), TEST_PREC));
}
...
TEST_F(SphericalJoint, TestCRBA)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(H_id.data(), H_crba.data(), multdof3_model.qdot_size * multdof3_model.qdot_size, TEST_PREC));
}
...
TEST_F(SphericalJoint, TestForwardDynamicsLagrangianVsABA)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(QDDot_lag.data(), QDDot_aba.data(), multdof3_model.qdot_size, TEST_PREC));
}
...
TEST_F(SphericalJoint, TestContactsLagrangian)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(emu_force_lagrangian.data(), sph_force_lagrangian.data(), 3, TEST_PREC));
}
...
TEST_F(SphericalJoint, TestContacts)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(emu_force_kokkevis.data(), sph_force_kokkevis.data(), 3, TEST_PREC));
}
...
TEST_F(SphericalJoint, TestEulerZYXvsEmulatedLagrangian)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(QDDot_emu.data(), QDDot_eulerzyx.data(), emulated_model.qdot_size, TEST_PREC));
}
...
TEST_F(SphericalJoint, TestEulerZYXvsEmulatedContacts)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(QDDot_emu.data(), QDDot_eulerzyx.data(), emulated_model.qdot_size, TEST_PREC * QDDot_emu.norm()));
...
TEST_F(SphericalJoint, TestJointTypeTranslationZYX)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(id_emulated.data(), id_3dof.data(), id_emulated.size(), TEST_PREC * id_emulated.norm()));
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEq(qddot_emulated.data(), qddot_3dof.data(), qddot_emulated.size()));
...
TEST_F(SphericalJoint, TestJointTypeEulerXYZ)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEq(model_emulated.X_base[3].E.data(), model_3dof.X_base[1].E.data(), 9));
    EXPECT_TRUE(unit_test_utils::checkArraysEq(model_emulated.v[3].data(), model_3dof.v[1].data(), 6));

    ForwardDynamicsLagrangian(model_emulated, q, qdot, tau, qddot_emulated);
    ForwardDynamicsLagrangian(model_3dof, q, qdot, tau, qddot_3dof);

    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(qddot_emulated.data(), qddot_3dof.data(), qddot_emulated.size(), TEST_PREC));
...
TEST_F(SphericalJoint, TestJointTypeEulerYXZ)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(model_emulated.X_base[3].E.data(), model_3dof.X_base[1].E.data(), 9, TEST_PREC));
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(model_emulated.v[3].data(), model_3dof.v[1].data(), 6, TEST_PREC));
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(id_emulated.data(), id_3dof.data(), id_emulated.size(), TEST_PREC));
...
TEST_F (Human36, TestUpdateKinematics)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(model_emulated->v[body_id_emulated[BodyPelvis]].data(), model_3dof->v[body_id_3dof[BodyPelvis]].data(), 6, TEST_PREC));
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(model_emulated->v[body_id_emulated[BodyThighRight]].data(), model_3dof->v[body_id_3dof[BodyThighRight]].data(), 6, TEST_PREC));
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(model_emulated->v[body_id_emulated[BodyShankRight]].data(), model_3dof->v[body_id_3dof[BodyShankRight]].data(), 6, TEST_PREC));
}
...
TEST_F (Human36, TestNonlinearEffects)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(nle_emulated.data(), nle_3dof.data(), nle_emulated.size(), TEST_PREC * nle_emulated.norm()));
}
...
TEST_F (Human36, TestContactsEmulatedLagrangianKokkevis)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(qddot_lagrangian.data(), qddot_kokkevis.data(), qddot_lagrangian.size(), TEST_PREC * qddot_lagrangian.norm()));
...
TEST_F (Human36, TestContactsEmulatedLagrangianSparse)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(qddot_lagrangian.data(), qddot_sparse.data(), qddot_lagrangian.size(), TEST_PREC * qddot_lagrangian.norm()));
...
TEST_F (Human36, TestContactsEmulatedLagrangianNullSpace)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(qddot_lagrangian.data(), qddot_nullspace.data(), qddot_lagrangian.size(), TEST_PREC * qddot_lagrangian.norm()));
...
TEST_F (Human36, TestContactsEmulatedMultdofLagrangian)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(qddot_emulated.data(), qddot_3dof.data(), qddot_emulated.size(), TEST_PREC * qddot_emulated.norm()));
...
TEST_F (Human36, TestContactsEmulatedMultdofKokkevisSparse)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(qddot_sparse.data(), qddot_kokkevis.data(), qddot_sparse.size(), TEST_PREC * qddot_sparse.norm()));
...
TEST_F (Human36, TestContactsEmulatedMultdofKokkevisMultiple)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(qddot_kokkevis.data(), qddot_kokkevis_2.data(), qddot_kokkevis.size(), TEST_PREC * qddot_kokkevis.norm()));
...
TEST_F (Human36, SolveMInvTimesTau)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(qddot_solve_llt.data(), qddot_minv.data(), model->dof_count, TEST_PREC * qddot_minv.norm()));
}
...
TEST_F (Human36, SolveMInvTimesTauReuse)
{
    ...
    for (unsigned int j = 0; j < 1; j++)
    {
        ...
        EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose(qddot_solve_llt.data(), qddot_minv.data(), model->dof_count, TEST_PREC * qddot_solve_llt.norm()));
    }
}
