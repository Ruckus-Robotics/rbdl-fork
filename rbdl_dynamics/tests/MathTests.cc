...
TEST (MathFixture,GaussElimPivot) {
  ...
  EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose (test_result.data(), x.data(), 3, TEST_PREC));
...
TEST (MathFixture,Dynamic_1D_initialize_value) {
  ...
  EXPECT_TRUE(unit_test_utils::checkArraysEq (test_values, myvector_10.data(), 10));
  delete[] test_values;
}
...
TEST (MathFixture,SpatialMatrix_Multiplication) {
  ...
  EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose (correct_result.data(), test_result.data(), 6 * 6, TEST_PREC));
