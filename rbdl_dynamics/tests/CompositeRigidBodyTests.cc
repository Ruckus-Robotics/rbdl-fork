...
TEST_F(FloatingBase12DoF, TestCRBAFloatingBase12DoFInverseDynamics) {
  ...
  EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose (H_crba.data(), H_id.data(), model->dof_count * model->dof_count, TEST_PREC));
}
...
TEST_F(CompositeRigidBodyFixture, TestCompositeRigidBodyForwardDynamicsSpherical) {
  ...
  EXPECT_TRUE(unit_test_utils::checkArraysEpsilonClose (H_ref.data(), H.data(), 9, TEST_PREC));
}
